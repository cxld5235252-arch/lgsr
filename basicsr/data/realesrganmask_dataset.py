...
class RealESRGANMaskDataset(data.Dataset):
    ...
    def __init__(self, opt, mode='training'):
        super(RealESRGANMaskDataset, self).__init__()
        self.opt = opt
        ...
        self.paths = os.listdir(self.gt_root)

        self.cf_paths, self.re_paths = [], []

        import random
        
        for _path in self.paths:
            if _path.count('IMG') > 0:
                self.cf_paths.append(_path)
            else:
                self.re_paths.append(_path)
        
        self.paths = self.cf_paths
        print(f'CameraFusion数量是{len(self.paths)}')
        num_re = int(len(self.paths)*0.2)

        self.re_paths = random.sample(self.re_paths, num_re)
        self.paths.extend(self.re_paths)
        print(f'数量是{len(self.paths)}')
        
        random.shuffle(self.paths)
        
        # self.tele_paths = sorted(os.listdir(self.tele_root))
        # self.lq_paths = sorted(os.listdir(self.lq_root))
        # self.mask_paths = sorted(os.listdir(self.mask_root))
    ...
    def __getitem__(self, index):
        ...
        lq_path = os.path.join(self.lq_root, self.paths[index].replace('gt','wide'))
        tele_path = os.path.join(self.tele_root, self.paths[index].replace('gt','tele'))
        mask_path = os.path.join(self.mask_root, self.paths[index].replace('gt','occ'))
        ...
        processed_image = 1.0-img_mask
        img_mask = processed_image
        ...
        if self.mode == 'training':
            ...
            img_gt, img_lq, img_tele, img_mask = augment(img_gt, self.opt['use_hflip'], self.opt['use_rot'], img_tele,img_lq,img_mask)
        ...
        return_d = {'gt': img_gt, 'tele' : img_tele, 'lq':img_lq, 'mask':img_mask, 'gt_path': gt_path}
        return return_d
