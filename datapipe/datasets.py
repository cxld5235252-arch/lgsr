...
from basicsr.data.realesrganmask_dataset import RealESRGANMaskDataset
...
def create_dataset(dataset_config):
    ...
    elif dataset_config['type'] == 'realesrganmask':
        dataset = RealESRGANMaskDataset(dataset_config['params'])
...
class BaseData(Dataset):
    def __init__(
        ...
        self.file_paths = []
        self.file_paths = sorted(os.listdir(dir_path))
        self.extra_dir_path = extra_dir_path
        self.mask_dir_path = mask_dir_path
        print('Wide dir:%s'%(dir_path))
        print('Ref dir:%s'%(extra_dir_path))
        self.file_paths = [os.path.join(dir_path, img_name) for img_name in self.file_paths]
        # self.extra_dir_path = [os.path.join(extra_dir_path, img_name) for img_name in self.extra_dir_path]
    ...
    def __getitem__(self, index):
        ...
        im_base = util_image.imread(str(im_path_base), chn='bgr', dtype='float32')
        im_target = self.transform(im_base)
        
        im_path_extra = Path(self.extra_dir_path) / Path(im_path_base).name.replace('jpg', 'png').replace("wide", "tele")
        im_extra = util_image.imread(str(im_path_extra), chn='bgr', dtype='float32')
        im_extra = self.transform(im_extra)

        im_path_mask = Path(self.mask_dir_path) / Path(im_path_base).name.replace('jpg', 'png').replace("wide", "occ")
        im_mask = util_image.imread(str(im_path_mask), chn='bgr', dtype='float32')
        im_mask = self.transform(im_mask)

        # img_gt = cv2.imread(str(im_path_base))
        # img_tele = cv2.imread(str(im_path_extra)) 
        # img_mask = cv2.imread(str(im_path_mask))

        # im_base = cv2.cvtColor(img_gt, cv2.COLOR_BGR2RGB)/255.
        # im_extra = cv2.cvtColor(img_tele, cv2.COLOR_BGR2RGB)/255.
        # img_mask = cv2.cvtColor(img_mask, cv2.COLOR_BGR2RGB)/255.

        processed_image = 1.0-im_mask
        im_mask = processed_image

        # imagenet_mean = np.array([0.485, 0.456, 0.406])
        # imagenet_std = np.array([0.229, 0.224, 0.225])
        # im_target = im_base - imagenet_mean
        # im_target = im_target / imagenet_std
        # img_tele = im_extra - imagenet_mean
        # img_tele = img_tele / imagenet_std
        
        # im_target = img2tensor([im_target], bgr2rgb=False, float32=True)[0]
        # img_tele = img2tensor([img_tele], bgr2rgb=False, float32=True)[0]
        # im_mask = img2tensor([im_mask], bgr2rgb=False, float32=True)[0]
